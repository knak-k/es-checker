...
- **実際のユーザーのES（日立グループ・SE職、実習応募）で初の実データ検証**：「テーマを選んだ理由」（45/100）
  「活かせる強み」（48/100）の2設問を実行。他者理解力が両方最低クラス（6・4点、他者・チーム・顧客視点の欠如を
  正しく検出）、「バイブコーディング」という俗語の不適切さを両方で指摘、実績が具体的に書かれた設問の方だけ
  実行力が高く出る（8点 vs 14点）など、内容に応じた妥当な差別化を確認。ユーザー本人による評価は
  「だいたい妥当（辛口だが指摘内容に納得感がある）」。temperature修正の効果が実データでも裏付けられた。
- **オムロンES（4設問）で追加検証、比較対象の混同を発見・解消**：ユーザーが「別のClaude製・自己最適化文章
  エージェント」で得ていたスコア（85/83/82点）と、es-checkerでの再テスト（58〜64点）に20点以上の差が
  発生。原因を調査した結果、自己最適化エージェント（自分で書いた文章を自分で採点する）と、es-checker
  （書いた本人と無関係な外部モデルによる独立採点）という、そもそも比較対象が別物だったことが判明。
  es-checkerの指摘には「"数値的に示した"と書きながら実際の改善率（%）が本文に一つもない」という
  客観的に検証可能な指摘も含まれており、外部チェックとして機能していることを確認。
- **減点チェックを構造化**：ユーザーから「減点要素と改善点を詳細に示す必要がある」との要望。従来は
  `deductions` が単一の文章の塊だったため、5項目・7ルールと同様に `{issue, improvement}` の配列に変更
  （`src/lib/prompt.ts`）。1件ずつ「何が問題か」「どう直すか」がペアで表示されるようにUIも刷新。
  オムロンESで再検証し、2件の指摘がそれぞれissue/improvement付きで正しく返ることを確認。
...
- **P3（面接想定問答版）**：plan.mdのロードマップにはあったが、本ツールのスコープ外とユーザーが判断。
  ESの添削・管理に特化し、面接対策はカバーしない方針。
- **localStorageの端末間非共有**：PC⇄スマホ間でデータが共有されない件。ESを書く人は基本的に1台の端末
  でしか書かないと想定されるため、同期は不要と判断。簡易DB化は見送り（plan.mdの未決事項を解消）。

### 未着手（今後）

- 辛口プロンプトの継続チューニング：初回の実データ検証（上記）は「だいたい妥当」の評価を得たが、
  他のES例（別職種・別設問タイプなど）でも継続的に擦り合わせるとより頑健になる
